...
def load_model():
    ...
    return GLiNER.from_pretrained("koptelovmax/agroner_nuner_span")
...
def set_header():
    ...
    st.markdown(
        ...
        <div class="container">
            ...
            <p class="logo-text">ValoNER <span style="color:grey;"><font size="3">named-entities in agro-waste valorization</font></span></p>
        </div>
        """,
        unsafe_allow_html=True
    )
...
def main():
    ...
    # Load model for computing semantic similarity:
    model_emb = load_embeddings()
    ...
    with col_2:
        ...
        if threshold_flag:
            ...
            threshold_input = st.slider("Select a threshold (the lower the threshold, the more entities will be discovered, and vice versa):", min_value=0.1, max_value=0.9, value=0.4, step=0.01, disabled=True)
        else:
            ...
            threshold_input = st.slider("Select a threshold (the lower the threshold, the more entities will be discovered, and vice versa):", min_value=0.1, max_value=0.9, value=0.4, step=0.01, disabled=False)
    ...
    if st.button('Submit'):
        ...
        colors = ["red","blue","green","grey","violet","orange","rainbow","gray","blue"]
        label_dic = {}
        ...
        for l in range(len(text_lines)):
            ...
            if threshold_flag:
                ...
                pred_entities = model.predict_entities(text_lines[l], labels, threshold=0.4)
            else:
        ...
        entity_embedding = model_emb.encode(entities) # get embeddings for entities
        text_embedding = model_emb.encode(text_lines) # get embeddings for segments
        sim_scores = util.dot_score(entity_embedding,text_embedding) # compute similarity matrix
        
        # Get similarities of segments containing desired entities:
        sem_relev_list= []
        sem_relev_ent_list= []
        for i in range(len(entities)):
            sem_relev_list.append((np.mean([sim_scores[i][j] for j in range(len(text_lines)) if entity_frequency[i][j] != 0]),entities[i],colors[i]))
            sem_relev_ent_list.append((np.sum([entity_frequency[i][j]*sim_scores[i][j] for j in range(len(text_lines)) if entity_frequency[i][j] != 0])/np.sum(entity_frequency[i]),entities[i],colors[i]))
        ...
        sem_relev_sorted = sorted(sem_relev_list, key=lambda tup: tup[0], reverse=True)
        sem_relev_ent_sorted = sorted(sem_relev_ent_list, key=lambda tup: tup[0], reverse=True)
        
        # TODO: add sliders to select if computing frequencies and relevance or not
        
        # Output the results:        
        text_to_output = "**Frequencies (segments):**"
        for i in range(len(entities)-1):
        ...
        text_to_output = "**Frequencies (entities):**"
        for i in range(len(entities)-1):
        ...
        text_to_output = "**Semantic relevance:**"
        for i in range(len(entities)-1):
            text_to_output += ":"+sem_relev_sorted[i][2]+"-background["+sem_relev_sorted[i][1]+"] " + str(np.round(sem_relev_sorted[i][0],2)) + ", "
        if len(entities) != 0:
            text_to_output += ":"+sem_relev_sorted[i+1][2]+"-background["+sem_relev_sorted[i+1][1]+"] " + str(np.round(sem_relev_sorted[i+1][0],2))
        st.markdown(text_to_output)
        
        text_to_output = "**Semantic relevance (weighted):**"
        for i in range(len(entities)-1):
            text_to_output += ":"+sem_relev_ent_sorted[i][2]+"-background["+sem_relev_ent_sorted[i][1]+"] " + str(np.round(sem_relev_ent_sorted[i][0],2)) + ", "
        if len(entities) != 0:
            text_to_output += ":"+sem_relev_ent_sorted[i+1][2]+"-background["+sem_relev_ent_sorted[i+1][1]+"] " + str(np.round(sem_relev_ent_sorted[i+1][0],2))
        st.markdown(text_to_output)
